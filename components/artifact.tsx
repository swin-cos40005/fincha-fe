...
function PureArtifact({
  ...
  votes,
  isReadonly,
  selectedVisibilityType, // eslint-disable-line @typescript-eslint/no-unused-vars
}: {
  chatId: string;
  input: string;
  ...
  const { getVisibleArtifact } = useActiveArtifacts();
  const visibleArtifact = getVisibleArtifact();
  
  // Use the artifact-specific hook for the visible artifact
  const { artifact, setArtifact, metadata, setMetadata } = useArtifactById(
    visibleArtifact?.documentId || null
  );
  ...
  return (
    <AnimatePresence>
      {visibleArtifact && artifact.documentId !== 'init' && (
        <motion.div
